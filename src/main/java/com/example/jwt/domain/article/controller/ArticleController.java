...
public class ArticleController {
    ...
    public RsData<WriteResponse> write(
            ...
            @Valid @RequestBody WriteRequest writeResquest
    ) {
        Member member = memberService.findByUsername(user.getUsername()).orElseThrow();
        RsData<Article> writeRs = articleService.write(member, writeResquest.getSubject(), writeResquest.getContent());
    ...
    public RsData<DeleteResponse> delete(
            @AuthenticationPrincipal User user,
            @PathVariable("id") Long id
    ){
        Member member = memberService.findByUsername(user.getUsername()).orElseThrow();
